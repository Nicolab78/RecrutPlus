package com.recrutplus.controller;
...
public class ApplicationController {
  ...
  public ResponseEntity<?> submitApplication(
      @RequestParam("data") String applicationData, @RequestParam("cv") MultipartFile cv) {

    try {
      CreateApplicationDTO createApplicationDTO =
          objectMapper.readValue(applicationData, CreateApplicationDTO.class);
  ...
  public ResponseEntity<?> getAllApplications(
    ...
    try {
      List<ApplicationDTO> applications =
          applicationService.getAllApplications(status, jobOfferId, email);
      return ResponseEntity.ok(applications);
    } catch (RuntimeException e) {
      return new ResponseEntity<>(e.getMessage(), HttpStatus.BAD_REQUEST);
    }
  ...
  public ResponseEntity<?> processApplication(
      @PathVariable Long id, @Valid @RequestBody ProcessApplicationDTO processApplicationDTO) {
    try {
  ...
  public ResponseEntity<?> updateApplication(
      @PathVariable Long id, @Valid @RequestBody UpdateApplicationDTO updateApplicationDTO) {
    try {
