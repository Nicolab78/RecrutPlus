...
import com.recrutplus.service.impl.JobOfferService;
...
public class JobOfferController {
  ...
  public ResponseEntity<?> searchJobOffers(
    ...
    try {
      List<JobOfferDTO> jobOffers =
          jobOfferService.searchJobOffers(keyword, specialty, contractType);
      return ResponseEntity.ok(jobOffers);
    } catch (RuntimeException e) {
      return new ResponseEntity<>(e.getMessage(), HttpStatus.BAD_REQUEST);
    }
  ...
  public ResponseEntity<?> updateJobOffer(
      @PathVariable Long id, @Valid @RequestBody UpdateJobOfferDTO updateJobOfferDTO) {
    try {
