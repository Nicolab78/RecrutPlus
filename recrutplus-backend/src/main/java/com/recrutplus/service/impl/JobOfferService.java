...
import com.recrutplus.service.interfaces.IJobOfferService;
...
public class JobOfferService implements IJobOfferService {
  ...
  @Override
  public List<JobOfferDTO> searchJobOffers(
      String keyword, Specialty specialty, ContractType contractType) {
    List<JobOffer> jobOffers =
        jobOfferRepository.findByIsActiveTrue().stream()
            .filter(
                jobOffer -> {
                  ...
                  if (keyword != null && !keyword.isEmpty()) {
                    ...
                    matches =
                        jobOffer.getTitle().toLowerCase().contains(lowerKeyword)
    ...
    return jobOffers.stream().map(this::mapToJobOfferDTO).collect(Collectors.toList());
  }

  @Override
  public List<JobOfferDTO> getJobOffersBySpecialty(Specialty specialty) {
    List<JobOffer> jobOffers =
        jobOfferRepository.findAll().stream()
  ...
  private JobOfferDTO mapToJobOfferDTO(JobOffer jobOffer) {
    return JobOfferDTO.builder()
        ...
        .applicationsCount(
            jobOffer.getApplications() != null ? jobOffer.getApplications().size() : 0)
        .build();
  }
