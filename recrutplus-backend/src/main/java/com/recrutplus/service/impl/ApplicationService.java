...
import com.recrutplus.model.Address;
...
import com.recrutplus.service.interfaces.IApplicationService;
...
public class ApplicationService implements IApplicationService {
  ...
  public ApplicationDTO submitApplication(
      CreateApplicationDTO createApplicationDTO, MultipartFile cv) {
    ...
    JobOffer jobOffer =
        jobOfferRepository
            .findById(createApplicationDTO.getJobOfferId())
    ...
    if (applicationRepository.existsByEmailAndJobOfferId(
        createApplicationDTO.getEmail(), createApplicationDTO.getJobOfferId())) {
  ...
  private String saveCV(Long applicationId, MultipartFile cv) throws IOException {
    return gridFsService.uploadCv(cv);
  }
  ...
  public List<ApplicationDTO> getAllApplications(String status, Long jobOfferId, String email) {
    ...
    return applications.stream().map(this::mapToApplicationDTO).collect(Collectors.toList());
  }
  ...
  public ApplicationDTO processApplication(Long id, ProcessApplicationDTO processApplicationDTO) {
    ...
    if (currentStatus == ApplicationStatus.EN_ATTENTE && newStatus != ApplicationStatus.EN_COURS) {
      throw new RuntimeException(
          "Une candidature en attente doit d'abord être mise en cours d'évaluation");
    }

    if (newStatus == ApplicationStatus.ENTRETIEN_TERMINE) {
      if (currentStatus != ApplicationStatus.ACCEPTE_ENTRETIEN) {
        throw new RuntimeException(
            "Seules les candidatures convoquées peuvent passer à 'Entretien terminé'");
      }
    ...
    if (newStatus == ApplicationStatus.EMBAUCHE
        || newStatus == ApplicationStatus.REFUSE_APRES_ENTRETIEN) {
      if (currentStatus != ApplicationStatus.ENTRETIEN_TERMINE) {
        throw new RuntimeException(
            "La candidature doit avoir le statut 'Entretien terminé' pour cette décision");
      }
    }

    if ((newStatus == ApplicationStatus.REFUSE
            || newStatus == ApplicationStatus.REFUSE_APRES_ENTRETIEN)
        && (processApplicationDTO.getComment() == null
            || processApplicationDTO.getComment().trim().isEmpty())) {
      ...
      CompletableFuture.runAsync(
          () -> {
            try {
              ...
              System.err.println(
                  "Erreur envoi email changement statut: " + emailError.getMessage());
            }
          });
    }
    ...
    return ApplicationDTO.builder()
        ...
        .address(
            application.getUser() != null
                ? mapToAddressDTO(application.getUser().getAddress())
                : null)
