...
import com.recrutplus.service.interfaces.IInterviewService;
  ...
  public InterviewDTO createInterview(CreateInterviewDTO dto) {
    ...
    if (dto.getType() == InterviewType.PRESENTIEL) {
      if (dto.getVisioLink() != null && !dto.getVisioLink().isEmpty()) {
        throw new RuntimeException(
            "Le lien visio n'est pas autorisé pour un entretien en présentiel");
      }
    }
    ...
    CompletableFuture.runAsync(
        () -> {
          try {
            Thread.sleep(5000);
            String interviewDateFormatted =
                saved.getInterviewDate().format(DateTimeFormatter.ofPattern("dd/MM/yyyy à HH:mm"));
            ...
            System.out.println(
                "Email invitation entretien envoyé en arrière-plan à : "
                    + finalApplication.getEmail());
          } catch (Exception e) {
              ...
              try {
                ...
                String message =
                    customNotes != null && !customNotes.trim().isEmpty()
                        ...
                        : "Votre entretien du "
                            + interviewDateFormatted
                            + " s'est bien déroulé. Nous reviendrons vers vous prochainement avec notre décision.";

                emailService.sendStatusChange(
                    finalInterviewTermine.getApplication().getEmail(),
                    finalInterviewTermine.getApplication().getFirstname()
                        + " "
                        + finalInterviewTermine.getApplication().getLastname(),
                    finalInterviewTermine.getApplication().getJobOffer().getTitle(),
                    ApplicationStatus.ENTRETIEN_TERMINE,
                    message);

                System.out.println(
                    "Email entretien terminé envoyé à : "
                        + finalInterviewTermine.getApplication().getEmail());
              } catch (Exception e) {
            ...
            () -> {
              try {
                ...
                emailService.sendInterviewCancellation(
                    ...
                    finalInterview.getApplication().getFirstname()
                        + " "
                        + finalInterview.getApplication().getLastname(),
                    finalInterview.getApplication().getJobOffer().getTitle(),
                    interviewDateFormatted,
                    reason);

                System.out.println(
                    "Email annulation entretien envoyé via update à : "
                        + finalInterview.getApplication().getEmail());
              } catch (Exception e) {
