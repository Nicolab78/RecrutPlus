...
class ApplicationServiceTest {
  ...
  void init() {
    ...
    validCv =
        new MockMultipartFile("cv", "cv.pdf", "application/pdf", "%PDF-valid content".getBytes());

    activeJobOffer = JobOffer.builder().id(1L).title("Dev Java").isActive(true).build();
  ...
  void submitApplication_shouldThrow_whenCvIsNull() {
    assertThrows(
        RuntimeException.class, () -> applicationService.submitApplication(validDTO, null));
  }
  ...
  void submitApplication_shouldThrow_whenCvIsEmpty() {
    ...
    assertThrows(
        RuntimeException.class, () -> applicationService.submitApplication(validDTO, emptyCv));
  }
  ...
  void submitApplication_shouldThrow_whenContentTypeIsWrong() {
    MultipartFile wrongContentType =
        new MockMultipartFile(
            "cv", "cv.pdf", "application/octet-stream", "%PDF-content".getBytes());
  ...
  void submitApplication_shouldThrow_whenJobOfferIsInactive() {
    JobOffer inactiveOffer = JobOffer.builder().id(1L).title("Dev Java").isActive(false).build();
  ...
  void processApplication_shouldThrow_whenApplicationNotFound() {
    ...
    assertThrows(
        RuntimeException.class, () -> applicationService.processApplication(1L, processDTO));
  }
  ...
  void processApplication_shouldThrow_whenEmbaucheFromWrongStatus() {
    ...
    processDTO = ProcessApplicationDTO.builder().status(ApplicationStatus.EMBAUCHE).build();
  ...
  void processApplication_shouldUpdateStatus_whenEnAttenteToEnCours() {
    ...
    verify(applicationRepository)
        .save(argThat((Application app) -> app.getStatus() == ApplicationStatus.EN_COURS));
  }
  ...
  void processApplication_shouldUpdateStatus_whenEntretienTermineWithCompletedInterview() {
    ...
    Interview completedInterview = Interview.builder().status(InterviewStatus.TERMINE).build();
    ...
    verify(applicationRepository)
        .save(argThat((Application app) -> app.getStatus() == ApplicationStatus.ENTRETIEN_TERMINE));
  }
}
